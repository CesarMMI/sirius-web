...
import { IEmpresa } from "../../pages/empresa/models/empresa";
        ...
        <ng-container *ngIf="responsive$ | async as responsive">
            <ng-container *ngIf="empresa$ | async as empresa; else loading">
                <div
                    [ngClass]="{
                        ...
                        'w-4 mx-auto flex gap-2': responsive.width >= 768
                    }"
                    class="p-2 surface-100 border-round-left-lg border-round-right-2xl"
                >
                    <span
                        class="text-center text-color font-medium flex-grow-0 w-4 mr-3 lg:mr-0 text-overflow-ellipsis"
                        >{{ empresa.xrazaoSocial }}</span
                    >
                    <span
                        class="text-center text-color font-medium flex-grow-1 w-7"
                        >{{ empresa.cnpj | cpfCnpj }}</span
                    >
                    <span
                        class="w-1 text-right cursor-pointer"
                        (click)="refreshClick()"
                    >
            ...
            <ng-template #loading>
                <div
                    ...
                    class="flex gap-2 p-2 surface-100 border-round-left-lg border-round-right-2xl"
                >
                    <p-skeleton
                        height="1.2rem"
                        class="flex-grow-0 w-4"
                    ></p-skeleton>
                    <p-skeleton
                        height="1.2rem"
                        class="flex-grow-1 w-7"
                    ></p-skeleton>
